...
router.post("/", requireToken, async (req, res, next) => {
  try {
    ...
    const newPerson = await db.Profile.create(req.body);
...
router.put("/:id", requireToken, async (req, res) => {
	try {
		...
		const updatedPerson = await db.Profile.findByIdAndUpdate(
			req.params.id,
			req.body,
			{ new: true }
		)
...
router.delete("/:id", requireToken, async (req, res, next) => {
  try {
    handleValidateOwnership(req, await db.Profile.findById(req.params.id));
    const deletedPerson = await db.Profile.findByIdAndRemove(req.params.id);
